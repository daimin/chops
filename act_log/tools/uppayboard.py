...
def update_reg_nopay(cur):
    ...
    cur.execute("select count(`reg_date`)as regcount,`reg_date` from userinfo group by `reg_date`")
    for regcount,reg_date in cur.fetchall():
        cur.execute("update reg_nopay set num=%d where `date`='%s'" %(regcount, reg_date))
    cur.execute(sql)
    for reg_date,rnum in cur.fetchall():
        ...
        cur.execute("update reg_nopay set num=%d where `date`='%s'" %(regcount[0]-rnum, reg_date.strftime("%Y-%m-%d")))
